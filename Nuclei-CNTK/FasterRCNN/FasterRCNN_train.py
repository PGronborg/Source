...
def train_model(image_input, roi_input, dims_input, loss, pred_error,
    ...
    epoch_train_loss2 = [0]*epochs_to_train
    ...
    for epoch in range(epochs_to_train):       # loop over epochs
        ...
        while sample_count < cfg["DATA"].EPOCH_SIZE:  # loop over minibatches in the epoch
            ...
            if sample_count%10==0:                                   # update model with it
                ...
                loss2 += float(trainer.loss_function.eval(data))
                count += 1
            sample_count += trainer.previous_minibatch_sample_count          # count samples processed so far
        epoch_train_loss[epoch] = loss / count
        epoch_train_loss2[epoch] = loss2 / count
        epoch_train_error[epoch] = error / count
    ...
    with open(cfg['VAL_PATH'], 'w') as csvfile:
        ...
        valwriter.writerow(['Count','Train Loss','Train Loss2','Train error','Val Loss','Val error'])
        for saver in range(early_stop_final_count):
            valwriter.writerow([saver,epoch_train_loss[saver],epoch_train_loss2[saver],epoch_train_error[saver],val_losses[saver],val_error[saver]])
